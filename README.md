...
永续回测的成交约定是信号确认后的下一根 K 线开盘价，并记录该根 K 线的实际成交时间；已知数据窗口的最后一根 K 线也会执行预定开盘成交，并对更早建立的仓位依次检查强平、止损和通道退出。持仓期间反向突破通道会逐根 K 线更新，风险退出优先于同根 K 线加仓；行情每有利移动 `0.5N` 可增加一个单位，最多 4 单位，并受 `--max-total-open-risk`（默认 4%）、可用保证金和合约规格约束。入场当根不使用未知盘中路径立即退出或加仓；幸存仓位才在期末收盘强制平仓，最终手续费和滑点也计入最大回撤。手续费、滑点、资金费率和近似强平仍分别计入报告。没有交易所风险档位数据时，报告会标明使用交易所无关的研究近似强平模型，不能当作实盘清算价。风险档位可选保存报价币计价的 `maintenance_amount`（兼容 `maintenance_deduction`、`deduction` 和 `cum` 别名）；配置该字段后，强平研究模型会按“档位维持保证金减扣减额”计算并使用独立的 v2 模型版本，旧档位仍保持原 v1 结果。

回测报告还会生成 `maintenance_margin_stress`：版本化历史快照如果冻结了风险档位，回测会复核完整校验和及来源绑定；只有 provider、symbol、官方接口参数、获取时间和版本均有效时才标记为官方档位，经过配置绑定的档位会单独标记，未知来源档位会被拒绝；`--maintenance-margin-tiers-json` 仅作为显式人工覆盖。系统会对照固定维持保证金率与分层维持保证金率下的收益、交易数、强平数、最大回撤和期末权益，差值口径固定为“分层减固定”。快照和命令行都没有经过核验的档位时该项明确标记为禁用，不会生成虚假的对照结果。命令行输出会在 Python 运行环境支持时统一使用 UTF-8，便于 Windows 终端和重定向日志稳定显示中文。
回测还会生成 `liquidation_fee_stress`，分别重跑当前配置、保守强平费用、双倍强平费用，以及“双倍强平费用 + 放大滑点/成交量冲击”的极端场景。默认保守费用率为 0.5%，可用 `--liquidation-fee-stress-rate` 修改；极端滑点倍数可用 `--liquidation-extreme-slippage-multiplier` 修改。强平发生时费用会作为额外费用真实扣减权益和单笔净盈亏，不再只用于调整预估强平价；报告同时记录累计强平费用，避免零费用基准低估尾部损失。
...
旧版风险档位仍可用 `maintenance_margin_tiers_by_provider.binance` 和 `.okx` 按 provider 配置。真实档位应优先写入 `maintenance_margin_tiers_by_provider_symbol.<provider>.<symbol>`，每个绑定包含 `tiers`、官方 `source`、带时区的 `effective_at` 和 `tier_version`；系统会计算或核对 `tier_checksum`。选择顺序是 provider+symbol、provider、全局档位、OKX 官方 isolated 数量档位、固定维持保证金率。OKX 的 `maxSz` 会结合合约 `ctVal` 转成基础币数量上限，避免误当成美元名义价值；可选档位请求只尝试一次且最多等待 10 秒。官方档位使用 `perp_runtime_cache` 中按币种隔离的最后有效快照：默认 6 小时内直接复用，刷新失败时最多允许使用 24 小时内的旧快照，并在元数据中明确标记 `fresh_cache` 或 `stale_fallback`、缓存年龄和刷新错误类别；缓存文件保存并重新校验完整 SHA-256，使用唯一临时文件、落盘同步和原子替换；旧版缓存读取时会重新计算完整校验和，并在下次官方刷新时升级格式。超过最大降级期限后仍会保留已有仓位管理、状态写回和健康告警，但默认暂停该 OKX 币种的新影子入场，避免固定费率样本混入官方档位样本；旧缓存剩余时间少于 `minimum_risk_tier_remaining_minutes_for_entry`（默认 120 分钟）时也暂停新入场。可通过 `allow_new_entries_on_stale_tier_cache` 和 `block_new_entries_on_risk_tier_unavailable` 调整策略，但不建议在正式研究 cohort 中关闭保护。`perp_shadow_stats.json` 会通过 `risk_tier_health` 汇总每个币种的缓存状态、年龄、剩余有效时间、连续刷新失败、入场门控和即将过期风险，GitHub Actions 正式扫描日志也会输出该摘要。自动故障转移创建样本时只冻结实际 provider 和 symbol 对应的档位、来源、版本、获取时间与校验和，后续配置或交易所档位变化不会重写旧交易。Binance 官方档位接口需要鉴权，模板仍保持空映射，不会用未经核验的数据填充。
...
每笔新样本还会冻结独立的 `risk_model_identity`，其摘要覆盖实际 provider、symbol、强平模型版本、档位作用域、档位版本、完整校验和与缓存状态，并保存二次 SHA-256 身份指纹。统计会分别按强平模型版本、档位版本、档位校验和、缓存状态和完整风险模型身份分组；因此官方档位更新、模型升级或 `stale_fallback` 降级样本不会再被静默合并为同一研究结论。旧样本在统计中明确归入 legacy/unversioned 分组，不会伪装成新模型样本。
...
仓库还包含独立的 `CoinPulse Monitor Health` 工作流，每 15 分钟检查 `monitor_health.json` 和 `perp_shadow_stats.json`。普通监控超过 20 分钟没有心跳、最近一次扫描失败，或永续统计超过 30 分钟未生成时，会通过 `SERVERCHAN_SENDKEY` 按故障组件变化发送一次失联告警；全部恢复后发送一次恢复通知。状态保存在 `monitor_alert_state.json`，不会在状态未变化时重复推送。健康记录还会保存当前多空单位、每个币种剩余容量和按止损估算的理论风险，供看板人工复核。该 dead-man 检查也会读取 `risk_tier_health`：官方档位进入旧缓存降级、不可用或接近过期时发送一次状态转换告警，连续失败只在 1/3/6/12 次等级和剩余 120/60/0 分钟等级变化时再次提醒，恢复后发送一次恢复通知；档位降级属于研究模型提示，不会让健康工作流失败。该 dead-man 检查与主扫描工作流分离，但仍运行在 GitHub Actions 内。
...
永续影子推送还会显示预定成交时间、信号有效期、预估保证金、预估最大亏损和组合剩余风险容量；成交/退出消息会显示实际成交时间。若需要避免多个渠道重复到达，可在配置中设置 `delivery.mode=primary_fallback`，并指定 `primary` 与 `fallback` 渠道名；默认 `broadcast` 保持所有已配置渠道同时发送。GitHub Actions 的 `workflow_dispatch` 现提供 `test_perp_push`，只发送一条确定性的永续研究测试消息，不读取行情、不写入运行状态。
