...
回测报告还会生成 `maintenance_margin_stress`：版本化历史快照如果冻结了官方风险档位，回测会优先使用并复核其中的版本与完整校验和；`--maintenance-margin-tiers-json` 仅作为显式人工覆盖。系统会对照固定维持保证金率与分层维持保证金率下的收益、交易数、强平数、最大回撤和期末权益，差值口径固定为“分层减固定”。快照和命令行都没有经过核验的档位时该项明确标记为禁用，不会生成虚假的对照结果。命令行输出会在 Python 运行环境支持时统一使用 UTF-8，便于 Windows 终端和重定向日志稳定显示中文。
